...
function Profile() {
	...
	useEffect(() => {
		if (isAdmin) {
			// fetchUserResume(token).then((res) => {
	...
	const changeAvatar = async (e) => {
		...
		try {
			...
			setLoading(true);
			const res = await axios.post(`${config.API_ENDPOINT}/api/upload_avatar`, formData, {
				headers: {
		...
		try {
			if (user._id !== id) {
				if (window.confirm("Are you sure you want to delete this account?")) {
					setLoading(true);
					await axios.delete(`${config.API_ENDPOINT}/user/delete/${id}`, {
						headers: { Authorization: token },
						withCredentials: true,
					});
					setLoading(false);
